...
- **Ingredients & Spices redesign — component phase** (2026-08-05, G1/G4) —
  ...
  `TieredIngredient`; China + Mexico hand-mapped as the iteration sandbox.
...
| # | Feature | Notes |
|---|---------|-------|
| 14 | **Personalized dish recommendations** | "6–8 dishes per country for you" + "what should I try next?" — both icebox recommendation ideas, one engine; the delivery surface is #1. Survey + profile may already cover 80% of the input side. The conversational **Food Preference Discovery** (Claude interview about textures, ingredients, aversions) is the deluxe input path — evaluate after familiarity weighting (#8) ships. |

### G2 🗺 Track & explore world cuisines

| # | Feature | Notes |
|---|---------|-------|
| 15 | **Cuisine passport / badges / streaks** | Confirmed Tier 3 (2026-08-05): gamification isn't relevant right now — progress plates (#3) cover the satisfying part. Revisit after plates ship, and skeptically (streaks punish normal eating habits in a personal diary). |
| 16 | **City-level data** | Restaurants by city, not just country. De-prioritized further since the standalone Restaurants section was cut. |
| 17 | **Log-from-map flow** | Demoted from Tier 2 (2026-08-05): the Eat & Drink "+ I tried this" flow is already fast, so the saved friction is ~2 clicks — possibly icebox material. Cheap alternative worth doing instead someday: deep-link the map hover card straight to the Eat & Drink tab. |

### G3 ✦ Know my own palate

| # | Feature | Notes |
|---|---------|-------|
| 18 | **Stats dashboard / annual recap** | Fun once there's more logged data. Folds in icebox items: rating distribution ("tough critic or generous rater?"), exploration trends graph, food-journey heat map. Natural trigger: build the recap in **December** with a year of data. |
| 19 | **Ingredient discovery** | Track new-to-you ingredients encountered as you log. Needs an ingredient⇄dish mapping to be meaningful — the #7 `keySpices` data is a head start. |

### G4 📖 Cultural depth

| # | Feature | Notes |
|---|---------|-------|
| 20 | **Regional flavor profiles + cross-region similarity** | Group each region's key spices into a categorized flavor profile, and "this region tastes like {other country}'s {region}". Deepens Culture & Regions; content-heavy (5 regions × 30 countries). The *personal* spice-preference half moved up to #7; what remains here is the static regional content. |
| 21 | **"Why this dish exists"** | Moved from Tier 2 (2026-08-05). Historical/cultural context snippet per dish — same pre-generated-content pipeline as dish twins (#4) and the spice data for #7; rides the #12 batch nearly free. |

### Foundation

| # | Feature | Notes |
|---|---------|-------|
| 22 | **Custom collections & tags** | Organization power tools; wait for logging volume. |
| 23 | **Seasonal highlights, meal companions** | Nice-to-haves. (Pronunciation moved into #1, the at-the-restaurant view, where it actually matters.) |
| 24 | **Supabase sync / mobile app** | Platform work; unlocks multi-device but changes nothing day-to-day until then. |
...
- **Per-dish/drink flavor match** — map the personal flavor profile down to individual foods and drinks: a taste-match indicator on each dish/drink card ("87% you"). Needs per-dish flavor data richer than spiceLevel — the `keySpices` generation in the #12 batch is the likely input; also the scoring ingredient for the at-the-restaurant view (#1) and personalized recs (#14), so it may get built as part of those. (G1/G3)

---

## Suggested next session

Tier 1 fully shipped (see Built). Next up: the **#1 at-the-restaurant view
design conversation** — the Tier 2 headline and the most direct build of the
thesis. China + Mexico remain the flavor-data sandbox until the #12 batch at
the MVP gate. Natural calendar trigger elsewhere: build the annual recap half
of #18 in **December**, when a year of data makes it fun.
